...
pytest_integration · try 1</pre>
    ...
    triage_timeout=45,            # seconds per call
        ...
        <p data-i18n="assistantBody">When an administrator enables report chat on the API server, the AI assistant button opens a centered, resizable window and answers questions about selected runs or the current dashboard filters. Its size is remembered per Airflow user. The scope shows the selected count and opens the full run list before a question is sent. Replies render safe Markdown, including tables that scroll inside the answer, source buttons open the exact reports used for the answer, and the same DAG read permissions are checked again for every question. The Context overview button shows the exact RBAC-filtered report-evidence block sent to the final provider, without the system prompt or chat history. Answers stream in as the model writes them, and while one is arriving <strong>Send</strong> becomes <strong>Stop</strong>: stopping keeps the text received so far, marks it incomplete and frees the assistant immediately. With a local model, the panel reports what the reducer is doing before the answer starts — model loading, chunks read, seconds left of the budget — so a long wait is visible rather than silent. Closing and reopening the window mid-answer is safe. Exact input, output and total token usage appears after an answer when the provider returns it. The header sums provider-reported total tokens across the whole chat session; refresh preserves the total and Clear chat resets it.</p>
        <div class="example">
...
mkdir -p /models
curl -fL 'https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF/resolve/main/qwen2.5-1.5b-instruct-q4_k_m.gguf?download=true' -o /models/qwen2.5-1.5b-instruct-q4_k_m.gguf
...
export AIRFLOW_PYTEST_ASSISTANT_MAX_OUTPUT_TOKENS=3072</pre>
        ...
        <p data-i18n="assistantInstall">The assistant-local extra installs the llama.cpp runtime only; it does not bundle a model. Grade any model before trusting it: measured on a fixed corpus, Qwen2.5-1.5B kept 76% of the required facts, 0.5B only 24% — worse than using no local model at all — and resident memory is roughly 2–2.4× the file size for every API-server process. The setting must point to a readable .gguf file, not a URL or directory. In Docker or Kubernetes, bake that file into the API-server image or mount /models read-only, then restart every API-server process. Workers and schedulers do not need the model.</p>
        <p data-i18n-html="assistantLimits"><strong>The chat shows the effective server values.</strong> Open <strong>Limits</strong> to see them as a vertical list. <code>CONTEXT_BYTES</code> is one shared report-evidence budget for every report in a direct-mode request, not a per-report allowance. Summary records are written first and keep the aggregate success counters; 100 summaries is a ceiling, so the byte budget can still omit the oldest summaries. Failure records are appended only whole. If the newest-first snapshot exceeds the budget, the answer is marked as context-limited and the request still completes. Chat history has its own 16,000-byte cap: it does not consume report evidence, but it is part of the final provider input and token usage. The final answer defaults to 3,072 output tokens; if the provider reaches that limit, the chat preserves the partial response and visibly warns that it may be incomplete. The API server reads the variables above at startup and publishes the resolved values through <code>GET /api/assistant/status</code>; the browser does not own a second configuration. <code>POST /api/assistant/health</code> goes further and checks that the models really answer, using one fixed probe with no report data; it is off until an operator sets <code>AIRFLOW_PYTEST_ASSISTANT_HEALTHCHECK</code>, because the probe is a billable call. In Docker Compose, put them under <code>environment</code> on the Airflow API-server service. All displayed data sizes use <code>KiB</code>, where <code>1 KiB = 1024 bytes</code>. Tunable data limits are bounded to safe ranges; invalid values fall back to defaults. Local chunks can be smaller than <code>CONTEXT_BYTES</code> so they fit <code>CONTEXT_N_CTX</code>. With a local model, one request may spend at most <code>LOCAL_BUDGET_SECONDS</code> (120 s by default) reducing the tree: past that the remaining chunks are skipped, the reduced facts still reach the provider, and the answer is marked as context-limited.</p>
        <div class="callout"><p data-i18n-html="assistantPrivacy"><strong>Know what leaves the server.</strong> A remote provider receives redacted report evidence, which can include failure tracebacks and a bounded part of captured stdout/stderr/log. With a local model, every readable run and test case is processed in chunks and hierarchically reduced; without one, a bounded direct snapshot is used. The local model costs API-server resources and does not replace the final provider. When an administrator has created the plugin's tables, a finished exchange is also stored for you on the server, so the chat survives closing the tab and follows you to another browser; only your question and the answer are kept, never the report evidence behind it, and only you can read or delete yours. The Chats button then lists your saved chats, New chat starts a fresh one without deleting anything, and deleting a chat asks first. Clear chat removes the stored copy of the chat you are reading. Without those tables the chat stays in this tab only.</p></div>
      </section>
...
(function () {
  var HELP_I18N = {
    en: {
      ...
      assistantBody: "When an administrator enables report chat on the API server, the AI assistant button opens a centered, resizable window and answers questions about selected runs or the current dashboard filters. Its size is remembered per Airflow user. Before a question is sent, the scope shows the selected count, opens the full run list, and explains how many RBAC-readable reports will be handled in local full-tree or direct bounded mode. Replies render safe Markdown, can be copied as their original Markdown, and show the UTF-8 bytes sent as system, user, report context, history and prompt structure. Context overview shows the exact RBAC-filtered report-evidence block sent to the final provider, without the system prompt or history. Answers stream in as the model writes them, and while one is arriving <strong>Send</strong> becomes <strong>Stop</strong>: stopping keeps the text received so far, marks it incomplete and frees the assistant immediately. With a local model, the panel reports what the reducer is doing before the answer starts — model loading, chunks read, seconds left of the budget — so a long wait is visible rather than silent. Closing and reopening the window mid-answer is safe. Exact input, output and total token usage appears after an answer when the provider returns it. The header sums provider-reported total tokens across the whole chat session; refresh preserves the total and Clear chat resets it. Source buttons open the exact reports used for the answer, and the same DAG read permissions are checked again for every question.",
